...
const deliveryHistory = [
  { id: "OT-K3X2P-AB12", from: "Fatuma Fresh Fish", to: "Near old fort, Lamu", amount: 80, date: new Date(Date.now() - 2 * 3600000), status: "delivered" },
  { id: "OT-M1Y9Q-CD34", from: "Lamu Market", to: "Shela seafront", amount: 120, date: new Date(Date.now() - 5 * 3600000), status: "delivered" },
  { id: "OT-P5Z7R-EF56", from: "Mama Mboga Lamu", to: "Manda Island jetty", amount: 150, date: new Date(Date.now() - 26 * 3600000), status: "delivered" },
];
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className={`rounded-3xl p-8 text-center transition-all duration-500 ${isOnline ? "bg-gradient-to-br from-green-400 to-teal" : "bg-gray-100"}`}>
        ...
        <button
          ...
          className={`px-8 py-3 rounded-2xl font-outfit font-bold text-sm transition-all active:scale-95 ${
            isOnline
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <StatCard label="Today's Earnings" value={formatKES(350)} icon={<DollarSign className="w-4 h-4" />} color="orange" />
        <StatCard label="This Week" value={formatKES(2140)} icon={<TrendingUp className="w-4 h-4" />} color="teal" />
        <StatCard label="Deliveries Today" value="4" icon={<Package className="w-4 h-4" />} color="navy" />
      ...
      {isOnline && (
        <div className="card border-l-4 border-l-orange">
          ...
          <div className="flex justify-between items-start mb-3">
            <div>
              <p className="font-outfit font-bold text-navy">Order #OT-R2X3M-IJ90</p>
              <p className="font-josefin text-gray-500 text-sm">Pick up from Fatuma Fresh Fish</p>
              <p className="font-josefin text-gray-500 text-sm">Deliver to: Near old fort, Lamu</p>
            </div>
